...
(defun net-error (output correct-output)
  ...
  ;;error metric ( 1/2 SUM_i (Ci - Oi)^2 )
  ;;error =  0.5 ( tr[c - o] . (c - o) )
  (let ((error  (map-m #'- correct-output output)))
    (e (scalar-multiply 1/2 (multiply (transpose error) error)) 1 1))
)


;; IMPLEMENT THIS FUNCTION
(defun forward-propagate (datum v w)
  "Returns as a vector the output of the OUTPUT units when presented the datum as input."

  ; The forward pass rules are (in this order):
  ; h = sigmoid[v . i] = (map-m #'sigmoid (multiply v i))
  ; o = sigmoid[w . h] = (map-m #'sigmoid (multiply w h))
  (let ((i (first datum)))
    (map-m #'sigmoid (multiply w (map-m #'sigmoid (multiply v i)))))
)
...
(defun back-propagate (datum alpha v w)
  ...
  (let* ((c (second datum))
	 (h (map-m #'sigmoid (multiply v (first datum))))
	 (o (map-m #'sigmoid (multiply w h))))
    (let* ((odelta (e-multiply (e-multiply (subtract c o) o) (subtract-from-scalar 1 o)))
      ...
      ;;insert error checking if matrix has changed sizes
      (list v w)))
)
...
(defun net-build (data num-hidden-units alpha initial-bounds max-iterations modulo &optional print-all-errors)
  ...
  (let* ((in-size (length (first (first data))))
	 (out-size (length (second (first data))))
	 (v (make-random-matrix num-hidden-units in-size initial-bounds))
	 (w (make-random-matrix out-size num-hidden-units initial-bounds))
	 (o '()) (tmp '()) (i 0) (worst 1000000) (mean 0) (mod modulo))
    (loop
       do
	 (progn
	   (setf data (shuffle data))
	   ; for each element do back propagation and error checking
	   (dotimes (n (length data))
	     (setf o (forward-propagate (nth n data) v w))
	     (setf tmp (back-propagate (nth n data) alpha v w))
	     (setf v (first tmp))
	     (setf w (second tmp))
	     (setf mean (+ mean (net-error (second (nth n data)) o)))
	     (if (= n 0) mean
		 (setf mean (/ mean n)))
	     (if (< (net-error (second (nth n data)) o) worst)
		 (setf worst (net-error (second (nth n data)) o))))
	   ; if its a mod iterative
	   (if (= i mod)
	       (progn
		 (print mod)
		 ; forward propagate
		 (dotimes (j (length data))
		   (forward-propagate (nth j data) v w))
		 ; if print all errors is true
		 (if print-all-errors
		     (progn
		       (print "Modulo Iteration")
		       ; find the error of each element
		       (dotimes (k (length data))
			 (format t "~%Element ~D error: ~D" k
				 (net-error (second (nth k data))
					    (forward-propagate (nth k data) v w))))))
		 (format t "~%Worst error: ~D" worst)
		 (format t "~%Mean error: ~D~%" mean)
		 (setf mod (+ mod modulo))))
	   (incf i))
       until
	 (or (= i max-iterations) (< worst *a-good-minimum-error*)))
...
(defun simple-generalization (data num-hidden-units alpha initial-bounds max-iterations)
  ...
  (let ((train '()) (test '()) (err 0) (avg 0) (worst 100))
    (setf data (convert-data data))
    (dotimes (i (length data))
      (if (< i (/ (length data) 2))
	  (setf train (append train (list (nth i data))))
	  (setf test (append test (list (nth i data))))))
    (let* ((in (length (first (first train))))
	   (out (length (second (first train))))
	   (v (make-random-matrix num-hidden-units in initial-bounds))
	   (w (make-random-matrix out num-hidden-units initial-bounds))
	   (tmp '()) (o '()) (idx 0) (nv '()) (nw '()))
      (loop while (or (< idx max-iterations) (< worst *a-good-minimum-error*))
	 do (progn
	      (setf train (shuffle train))
	      (dotimes (n (length train))
		(setf o (forward-propagate (nth n train) v w))
		(setf tmp (back-propagate (nth n train) alpha v w))
		(setf v (first tmp))
		(setf w (second tmp))
		(if (< (net-error (second (nth n train)) o) worst)
		    (setf worst (net-error (second (nth n train)) o))))
	      (incf idx)))
      (dotimes (k (length test))
	(setf tmp (back-propagate (nth k test) alpha v w))
	(setf nv (first tmp))
	(setf nw (second tmp))
	(setf err (+ err (net-error (second (nth k test))
				    (forward-propagate (nth k test) v w)))))
      (setf avg (/ err (length test)))
      (format t "~%Average error: ~D" avg)
      avg))
)
...
(simple-generalization *voting-records* 4 0.2 4 1000)  ;; pick appropriate values
(simple-generalization (convert-datum *voting-records*) 4 .02 1 1000))

;;; how well does this generalize usually?  Can you modify it to typically generalize better?
(simple-generalization *mpg* 5 1.1 6 2000) ;; pick appropriate values

;;; how well does this generalize usually?  Can you modify it to typically generalize better?
(simple-generalization *wine* 2 0.3 4 500)  ;; pick appropriate values
...
This project was much more interesting than last one. I think it may be the fact that 
I started earlier than usual or maybe because I actually took the time to play around 
with the code and understand what is happening. What is becoming more and more clear 
to me is that lisp is truly a very straightforward and elegant language and I can 
appreciate that.

After finishing the code for the net-build function I messed around a little bit 
the parameters, initially using the net build function on the nand and xor was taking 
a hot second, not too long but longer than I was comfortable and my error would be 
near 0.0… and everyone was talking about how for nand the code should converge so I 
tried to fix it and kind of snow balled. My back and forward propagation were so wrong, 
they were taking the datum and the v and w matrices and then returning a transformed 
datum and the same v and w matrices. After fixing that and fixing my net-error, which
was also somehow very wrong, I took out the multiply do times nested loops in my code 
which helped the runtime. I ran the nand and xor and the voting records multiple times. 
The error for my nand and or was extremely small so I was satisfied with that and my 
run time was better I didn’t have to wait 10 minutes for voting records. But I did 
notice that I did have to lower a good minimum error because the initial error was 
very low, and I wanted to check the part of the code that printed out the errors on 
the modulo iteration.

The simple generalization was its own beast. Honestly, I’m not super proud of it, 
couple of days ago I wrote it and moved on to k-fold but when I started running tests 
it wasn’t doing as I hoped. At first it was full of errors obviously but after I fixed 
it, I don’t think I am doing it right because my friends in the class have their data 
converging for nand and xor because that matrix is small, but I am getting extremely 
large errors. I ran my nand and xor and voting record multiple times (15 each) and the 
error I get is usually between 0.003 and 0.05 and I think that is not good enough. I 
tried to fix it, but I didn’t understand how to. I am taking half of the data and training 
it and then the testing it part throws me off a bit. I also tried to run some test cases 
using mpg and wine too, but they are also giving large errors. i also noticed when I 
change the alpha somehow the runtime gets worse, but the errors remain the same.
